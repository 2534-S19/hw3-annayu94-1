...
void initTimer(unsigned int timer, unsigned int preScaler, unsigned int count)
{
    ...
    Timer32_setCount(TIMER32_0_BASE, MAX_VALUE);
    Timer32_setCount(TIMER32_1_BASE, MAX_VALUE);


    // For the specified timer, configure the timer to repeat once it elapses.
    Timer32_startTimer(TIMER32_0_BASE, REPEAT);
    Timer32_startTimer(TIMER32_1_BASE, REPEAT);
}
